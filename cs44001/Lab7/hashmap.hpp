...
public:
   ...
   void rehash(size_t n); // will make a new numbuckets n
private:
...
hashmap<Key, Value, Compare, Hash>::insert(const Element &x)
{
   ...
   if (it == elems_[bucket].end())
   {
      ...
      return std::make_pair(&elems_[bucket].back(), true);
   }
...
hashmap<Key, Value, Compare, Hash>::erase(const Key &x)
{
   ...
   { // the element exists, erase it
      ...
      {
         size_t newbucket = bucket + 1;
         if (newbucket >= hash_.numBuckets())
         {
            newbucket = 0;
         }
         while (elems_[newbucket].empty())
         {
            // if (elems_[newbucket] == *(elems_.end()-1))
            // {
            //    return std::make_pair(nullptr, true);
            // }
            ++newbucket;
         ...
         return std::make_pair(&(*elems_[newbucket].begin()), true);
      }
   }
   else{
